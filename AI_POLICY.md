...
This project doesn't ban AI tools, but it does ban AI slop. This document draws the line.
...
- **Fabricated bug reports.** "Bugs" that are actually a model misunderstanding this project's syntax or semantics, submitted without first reproducing the behavior yourself with the `rl-embed` facade or the `rl-tests` harness.
...
- If a PR touches stdlib registration, verify both touchpoints in [CONTRIBUTING.md](CONTRIBUTING.md#adding-a-stdlib-function) (the `#[native_fn]` implementation in `rl-std` and the module registration in `rl-vm`) actually exist and match - this is exactly the kind of multi-file wiring that's easy for a model to under-cover or invent.
